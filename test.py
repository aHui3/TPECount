...
from model import TPECount
from visualize import save_density_map,save_heatmap,show_heatmap
import open_clip
...
def get_args_parser():
    ...
    parser.add_argument(
        "--resume",
        default="./checkpoints/checkpoint-tpecount.pth",
    ...
    parser.add_argument(
        "--img_dir",
        default="./datasets/FSC147_384_V2/images_384_VarV2",
        help="directory containing images from FSC-147",
    )

    parser.add_argument(
        "--FSC147_anno_file",
        default="./datasets/FSC147_384_V2/annotation_FSC147_384.json",
        help="name of file with FSC-147 annotations",
    )

    parser.add_argument(
        "--FSC147_D_anno_file",
        default="./FSC-147-D.json",#FSC-147-D_deletethe.json
        help="name of file with FSC-147-D",
    )

    parser.add_argument(
        "--data_split_file",
        default="./datasets/FSC147_384_V2/Train_Test_Val_FSC_147.json",
...
def main(args):
    ...
    for data_iter_step, (samples, gt_dots, test_clip_text_input) in enumerate(
        metric_logger.log_every(data_loader_test, print_freq, header)
    ):

        samples = samples.to(device, non_blocking=True)
        gt_dots = gt_dots.to(device, non_blocking=True).half()
        test_clip_text_input = test_clip_text_input.to(device, non_blocking=True)
        ...
        with open("log.txt", "a") as f:
            f.write(f"{data_iter_step}/{len(data_loader_test)}: pred_cnt: {pred_cnt},  gt_cnt: {gt_cnt},  AE: {cnt_err},  SE: {cnt_err ** 2} \n")
